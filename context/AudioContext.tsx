...
export function AudioProvider({ children }: any) {
	...
	// Music players
	const musicPlayers = {
		menu_music: useAudioPlayer(sounds.menu_music),
	...
	const sfxPlayers = {
		...
		wrong_sfx: useAudioPlayer(sounds.wrong_sfx),
	};
	...
	useEffect(() => {
		musicPlayers.menu_music.volume = musicVolume / 100;
		musicPlayers.menu_music.loop = true; // ✅ Loop the body
	...
	const playMusic = (musicName: string) => {
		...
		musicPlayers.menu_music.pause();
		musicPlayers.map_bg_music.pause();
		musicPlayers.review_music.pause();

		if (musicName === "menu_music") {
			// Play intro first
			musicPlayers.menu_music.seekTo(0);
			musicPlayers.menu_music.play();
	...
	return (
		<AudioContext.Provider value={{ playSound, playMusic }}>
			{children}
